...
public class StatsCommand implements CommandExecutor {
    ...
    private int getStatAtLevel(Character character, Stat stat, int level) {
        ...
        if (classesPluginProvider != null) {
            ...
            return (int) Math.round((((150D + (20D * (double) clazz.getStatBonus(stat) + (character instanceof CharacterImpl ? ((CharacterImpl) character).getStatPoints(stat) : 0) + character.getRace().getStatBonus(stat))) * (double) level) / 100D) + 5D);
        }
        return 0;
    }
