...
public class DrawbridgePlayerInteractListener implements Listener {

    @EventHandler
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            if (event.getClickedBlock().getState() instanceof Sign && event.getClickedBlock().getRelative(BlockFace.DOWN).getState() instanceof Sign) {
                Sign sign1 = (Sign) event.getClickedBlock().getState();
                Sign sign2 = (Sign) event.getClickedBlock().getRelative(BlockFace.DOWN).getState();
